...
export default function TemporaryDrawer({ showFilter, setShowFilter }) {
  ...
  const list = (anchor) => (
    ...
    <button
      className='fixed top-[15px] right-[15px] text-white rounded-full hover:bg-gray-500 p-1'
      ...
      {['right'].map((anchor) => (
        <React.Fragment key={anchor}>
          ...
          <Drawer
            ...
            onClick={(e) => {
              if(e.clientX <= screenWidth - sheetWidth || (e.clientY >= 20 && e.clientY <= 44 && e.clientX >= screenWidth - 34 && e.clientX <= screenWidth - 10)) setShowFilter(false);
            }}
          >
            {list(anchor)}
          </Drawer>
